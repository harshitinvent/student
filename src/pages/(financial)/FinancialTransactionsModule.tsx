...
import { getAllStudents, createTransaction, exportFinancialData, getStudentTransactions, handleAPIError } from '../../services/financialAPI';

export default function FinancialTransactionsModule() {
    ...
    useEffect(() => {
        ...
        if (Array.isArray(students)) {
            const stats = {
                ...
                totalRevenue: students.reduce((sum, s) => sum + s.total_charges, 0),
                totalOutstanding: students.reduce((sum, s) => sum + s.outstanding_balance, 0),
                studentsWithBalance: students.filter(s => s.outstanding_balance > 0).length
            };
    ...
    const filteredStudents = Array.isArray(students) ? students.filter(student => {
        ...
        const matchesProgramme = programmeFilter === 'ALL' || student.student.programme === programmeFilter;
        const matchesSemester = semesterFilter === 'ALL' || student.student.semester === semesterFilter;
        const matchesStatus = statusFilter === 'ALL' || student.account.status === statusFilter;
    ...
    const handleAddTransaction = async () => {
        ...
        try {
            await createTransaction({
                ...
                cheque_number: transactionForm.cheque_number,
                transaction_date: transactionForm.transaction_date,
                attachment: transactionForm.attachment
            });
    ...
    const getUniqueProgrammes = () => {
        ...
        const programmes = students.map(s => s.student.programme);
        return ['ALL', ...Array.from(new Set(programmes))];
    };

    const getUniqueSemesters = () => {
        if (!Array.isArray(students)) return ['ALL'];
        const semesters = students.map(s => s.student.semester);
        return ['ALL', ...Array.from(new Set(semesters))];
    };

    const handleExportData = async () => {
        try {
            const result = await exportFinancialData('excel', {
            ...
            link.download = `financial_transactions_${new Date().toISOString().split('T')[0]}.xlsx`;
    ...
    if (loading) {
        return (
            <div className="flex items-center justify-center h-64">
                <div className="text-lg">Loading financial transactions...</div>
            </div>
        );
    }
    ...
    return (
        <div className="relative">
            <PageTitleArea title="Financial Transactions Module">
                <div className="flex items-center gap-12">
                    ...
                    <Button
                        ...
                        style="transparent"
                    >
                        <HugeiconsIcon icon={DownloadIcon} className="size-16" />
                        Export
                    </Button>
                    <Button
                        ...
                        style="transparent"
                    >
                        <HugeiconsIcon icon={RefreshIcon} className="size-16" />
                        Refresh
                    </Button>
                    <Button
            ...
            <div className="px-24 py-16 max-md:px-16">
                {/* Summary Statistics */}
                <div className="grid gap-24 mb-32 md:grid-cols-2 lg:grid-cols-4">
                    <div className="bg-white rounded-16 border border-linePr p-24">
                        <div className="flex items-center justify-between mb-16">
                            <h3 className="text-16 font-medium text-textHeadline">Total Students</h3>
                            <HugeiconsIcon icon={UserIcon} className="size-20 text-blue-500" />
                        </div>
                        <p className="text-24 font-bold text-blue-600">{summaryStats.totalStudents}</p>
                        <p className="text-12 text-textDescription mt-8">Active accounts</p>
                    </div>

                    <div className="bg-white rounded-16 border border-linePr p-24">
                        <div className="flex items-center justify-between mb-16">
                            <h3 className="text-16 font-medium text-textHeadline">Total Revenue</h3>
                            <HugeiconsIcon icon={CoinsIcon} className="size-20 text-green-500" />
                        </div>
                        <p className="text-24 font-bold text-green-600">
                            {formatCurrency(summaryStats.totalRevenue)}
                        </p>
                        <p className="text-12 text-textDescription mt-8">All time charges</p>
                    </div>

                    <div className="bg-white rounded-16 border border-linePr p-24">
                        <div className="flex items-center justify-between mb-16">
                            <h3 className="text-16 font-medium text-textHeadline">Outstanding</h3>
                            <HugeiconsIcon icon={AlertIcon} className="size-20 text-red-500" />
                        </div>
                        <p className="text-24 font-bold text-red-600">
                            {formatCurrency(summaryStats.totalOutstanding)}
                        </p>
                        <p className="text-12 text-textDescription mt-8">Total outstanding</p>
                    </div>

                    <div className="bg-white rounded-16 border border-linePr p-24">
                        <div className="flex items-center justify-between mb-16">
                            <h3 className="text-16 font-medium text-textHeadline">With Balance</h3>
                            <HugeiconsIcon icon={AlertIcon} className="size-20 text-orange-500" />
                        </div>
                        <p className="text-24 font-bold text-orange-600">
                            {summaryStats.studentsWithBalance}
                        </p>
                        <p className="text-12 text-textDescription mt-8">Students with dues</p>
                    </div>
                </div>
                ...
                <Table
                    titles={[
                        ...
                        { icon: MortarboardIcon, title: 'Programme & Semester' },
                        { icon: CoinsIcon, title: 'Total Fees & Due' },
                        { icon: CalendarIcon, title: 'Last Payment' },
                        { icon: ClockIcon, title: 'Next Due Payment' },
                        { icon: EditIcon, title: 'Actions' },
                    ]}
                    gridClassName="md:grid-cols-[1.5fr_1.5fr_1fr_1fr_1fr_0.5fr]"
                >
                    {filteredStudents.map((student) => (
                        ...
                        >
                            <div>
                                ...
                                <div className="flex items-center gap-12">
                                    <Avatar className="size-32" src={student.student.avatar_url} />
                                    <div>
                                        <p className="text-textHeadline/80 text-14 font-medium">
                                            {student.student.first_name} {student.student.last_name}
                                        </p>
                                        <p className="text-textDescription text-12">
                                            {student.student.student_id} • {student.student.email}
                                        </p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div>
                                    ...
                                    <p className="text-textDescription text-12">
                                        {student.student.semester} • {student.student.academic_year}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div>
                                    <p className="text-textHeadline/80 text-14 font-medium">
                                        {formatCurrency(student.current_semester.total_fees)}
                                    </p>
                                    <p className={`text-12 font-medium ${student.current_semester.total_due > 0 ? 'text-red-600' : 'text-green-600'}`}>
                                        Due: {formatCurrency(student.current_semester.total_due)}
                                    </p>
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div>
                                    {student.current_semester.last_payment ? (
                                        <>
                                            <p className="text-textHeadline/80 text-14 font-medium">
                                                {formatCurrency(student.current_semester.last_payment.amount)}
                                            </p>
                                            <p className="text-textDescription text-12">
                                                {formatDateTime(student.current_semester.last_payment.date)}
                                            </p>
                                            <p className="text-textDescription text-12">
                                                {student.current_semester.last_payment.mode}
                                            </p>
                                        </>
                                    ) : (
                                        <p className="text-textDescription text-12">No payments</p>
                                    )}
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div>
                                    {student.current_semester.next_due_payment ? (
                                        <>
                                            <p className="text-textHeadline/80 text-14 font-medium">
                                                {formatCurrency(student.current_semester.next_due_payment.amount)}
                                            </p>
                                            <p className="text-textDescription text-12">
                                                Due: {formatDate(student.current_semester.next_due_payment.due_date)}
                                            </p>
                                        </>
                                    ) : (
                                        <p className="text-textDescription text-12">No pending dues</p>
                                    )}
                                </div>
                            </div>
                ...
                {filteredStudents.length === 0 && (
                    <div className="text-center py-32">
                        ...
                        <p className="text-textDescription">No students found matching your criteria</p>
                    </div>
                )}
            </div>
                        ...
                        <div className="rounded-32 max-md:rounded-24 bg-bgSec relative flex max-h-full w-full max-w-1000 flex-col overflow-hidden">
                            <div className="border-linePr flex items-center justify-between border-b p-24">
                                <div>
                                    ...
                                    <p className="text-14 text-textDescription">
                                        {selectedStudent.student.programme} • {selectedStudent.student.semester}
                                    </p>
                                </div>
                                <button
                            ...
                            <div className="flex-1 overflow-y-auto p-24">
                                ...
                                <div className="grid gap-24 mb-32 md:grid-cols-3">
                                    <div className="bg-white rounded-16 border border-linePr p-24">
                                        <h4 className="text-16 font-medium text-textHeadline mb-16">Current Semester</h4>
                                        <div className="space-y-12">
                                            <div className="flex justify-between">
                                                <span className="text-14 text-textDescription">Total Fees:</span>
                                                <span className="text-14 font-medium">{formatCurrency(selectedStudent.current_semester.total_fees)}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-14 text-textDescription">Total Paid:</span>
                                                <span className="text-14 font-medium text-green-600">{formatCurrency(selectedStudent.current_semester.total_paid)}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-14 text-textDescription">Total Due:</span>
                                                <span className="text-14 font-bold text-red-600">{formatCurrency(selectedStudent.current_semester.total_due)}</span>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="bg-white rounded-16 border border-linePr p-24">
                                        ...
                                        <div className="space-y-12">
                                            <div className="flex justify-between">
                                                ...
                                                <span className={`text-14 font-medium ${getStatusColor(selectedStudent.account.status)}`}>
                                                    {selectedStudent.account.status.replace('_', ' ')}
                                                </span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-14 text-textDescription">Student ID:</span>
                                                <span className="text-14 font-medium">{selectedStudent.student.student_id}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-14 text-textDescription">Enrollment:</span>
                                                <span className="text-14 font-medium">{formatDate(selectedStudent.student.enrollment_date)}</span>
                                            </div>
                                        </div>
                                    </div>

                                    <div className="bg-white rounded-16 border border-linePr p-24">
                                        <h4 className="text-16 font-medium text-textHeadline mb-16">Payment Summary</h4>
                                        <div className="space-y-12">
                                            <div className="flex justify-between">
                                                <span className="text-14 text-textDescription">All Time Charges:</span>
                                                <span className="text-14 font-medium text-red-600">{formatCurrency(selectedStudent.total_charges)}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-14 text-textDescription">All Time Payments:</span>
                                                <span className="text-14 font-medium text-green-600">{formatCurrency(selectedStudent.total_payments)}</span>
                                            </div>
                                            <div className="flex justify-between">
                                                <span className="text-14 text-textDescription">Outstanding:</span>
                                                <span className="text-14 font-bold text-red-600">{formatCurrency(selectedStudent.outstanding_balance)}</span>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                    ...
                                    <div className="p-24">
                                        {selectedStudent.recent_transactions.length === 0 ? (
                                            ...
                                            <div className="space-y-16">
                                                {selectedStudent.recent_transactions.map((transaction) => (
                                            ...
                                            >
                                                ...
                                                {students.map(student => (
                                                    <option key={student.student.id} value={student.student.id}>
                                                        {student.student.first_name} {student.student.last_name} - {student.student.student_id}
                                                    </option>
                                                ))}
                                            </select>
                                        </div>
