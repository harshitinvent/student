...
export default function AssignmentCanvas() {
  ...
  return (
    <div
      className={
        'grid h-full grid-cols-[15rem_1fr_15rem] gap-24 overflow-y-auto pt-20'
      }
    >
      <div
        className={
          'bg-bgPr rounded-20 border-linePr sticky top-0 flex h-full max-h-[calc(100vh-2.5rem)] flex-col justify-between self-start border'
        }
      >
        <div>
      ...
      <div className={'flex flex-col items-center gap-20 pb-96'}>
        <div className={'border-linePr w-full flex-1 shrink-0 border-b pb-20'}>
          <div
            className={
          ...
          >
            <h1
              className={'text-h5 text-textHeadline p-4 font-medium'}
              contentEditable
                ...
                {questions.map(({ title, options, answer }, i) =>
                  i >= 4 * activePage && i < 4 + 4 * activePage ? (
                    <div key={`question-${i}`} className={'grid w-full gap-16'}>
                      <p
                        className={
                          'text-h6 text-textHeadline flex items-center justify-start gap-12 font-medium'
                        }
                      >
                        <span
                          className={
        ...
        {requestStatus === 'success' && (
          <div
            className={
              'border-linePr rounded-20 absolute bottom-20 flex h-56 items-center gap-8 border bg-white px-8'
            }
          >
            <IconButton
      ...
      <div
        className={
          'rounded-20 border-linePr sticky top-0 self-start border bg-[#FCFCFC]'
        }
      >
        <div className={'flex items-center justify-between gap-4 p-12 pl-16'}>
          <div className={'flex items-center justify-start'}>
            <MemberAvatars />
          </div>
          <Button style={'gray'} onClick={() => setOpenShareModal(true)}>
            Share
          </Button>
        </div>
...
export function AnswerChoiceItem({
  ...
  return (
    <div
      className={`bg-bgSec rounded-12 border-linePr hover:shadow-s1 flex cursor-pointer items-center gap-17 border p-10 duration-300`}
      onClick={() => {
        onClick?.();
      }}
    >
