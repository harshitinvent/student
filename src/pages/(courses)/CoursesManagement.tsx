...
async function fetchCourses(search = '', page = 1, pageSize = 10): Promise<{ courses: Course[]; total: number }> {
...
async function fetchPrograms(): Promise<Array<{
    ID: string;
    name: string;
    degree_type: string;
...
export default function CoursesManagementPage() {
    ...
    const handleAdd = () => { setEditCourse(null); setModalOpen(true); };
    const handleEdit = (course: Course) => { setEditCourse(course); setModalOpen(true); };
    const handleModalClose = () => { setModalOpen(false); setEditCourse(null); };
    ...
    const openConfirmModal = (action: 'delete' | 'activate' | 'deactivate', course: Course) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            render: (status: 'Active' | 'Retired') => (
                <Tag color={status === 'Active' ? 'green' : 'red'} icon={status === 'Active'}>
                    {status}
                </Tag>
            ),
        },
                ...
                return (
                    <div>
                        {prerequisites.length > 0 ? (
                            ...
                            <span className="text-gray-400 text-sm">No prerequisites</span>
                        )}
                    </div>
                );
            },
        ...
        },
        {
            ...
            render: (_: any, course: Course) => (
                <div className="flex gap-3">
                    <Button className="table-action-button" onClick={() => handleEdit(course)} ><img src={EditIcon} alt="Edit" /></Button>
                    <Button
                        className="table-action-status-button"
                        icon={course.is_active ? <StopOutlined /> : <CheckCircleOutlined />}
                        onClick={() => openConfirmModal(course.is_active ? 'deactivate' : 'activate', course)}
                    >
                        {course.is_active ? 'Deactivate' : 'Activate'}
                    </Button>
                    <Button className="table-action-button" onClick={() => openConfirmModal('delete', course)} ><img src={DeleteIcon} alt="delete" /></Button>
                </div>
            ),
        },
    ];
