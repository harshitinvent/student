...
import { PlusOutlined, EditOutlined, DeleteOutlined, CheckCircleOutlined, StopOutlined, ClockCircleOutlined, BookOutlined, SearchOutlined } from '@ant-design/icons';
...
async function fetchPrograms(search = '', page = 1, pageSize = 10): Promise<{ programs: Program[]; total: number }> {
...
async function updateProgram(id: string, data: {
    name: string;
    degree_type: 'B.S.' | 'B.A.' | 'M.S.' | 'M.A.' | 'Ph.D.' | 'Associate';
    department_id: string;
    total_credits_required: number;
...
export default function ProgramsManagementPage() {
            ...
            .then(({ programs, total }) => {
                ...
                if (degreeTypeFilter) {
                    filteredPrograms = programs.filter(program =>
                        program.degree_type === degreeTypeFilter
                    );
                }
    ...
    const handleAdd = () => { setEditProgram(null); setModalOpen(true); };
    const handleEdit = (program: Program) => { setEditProgram(program); setModalOpen(true); };
    const handleModalClose = () => { setModalOpen(false); setEditProgram(null); };
        ...
        },
        {
            ...
            render: (_, record: Program) => (
                <div className="flex gap-2">
                    ...
                    ><img src={BookMarkIcon} alt="bookmark" /></Button>
                    <Button
                        className="table-action-button"
